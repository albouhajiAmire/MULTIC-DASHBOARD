...
function AdminProfil() {
  ...
  const handleSubmit = (e) => {
    ...
    for (const key in profilData) {
      if (profilData[key] === "") {
        ...
        toast.error(
          `${key}: this field must be two or  more then two character`
        );
        return;
      }
    }
    // dispatch({
    //   type: ME,
    //   payload: { ...user, ...profilData },
    // });
    setLoading(true);

    EditAccount(user._id, profilData, authorization)
      .then(({ data }) => {
        if (!data.err) {
          setLoading(false);
  ...
  const handleChangePass = (e) => {
    ...
    ChangePassword(user._id, passwordData, authorization)
      .then(({ data }) => {
        if (!data.err) {
          setLoading(false);
                    ...
                    <form onSubmit={handleSubmit}>
                      <div className="row mb-3">
                        ...
                        <div className="col-md-8 col-lg-9">
                          <img src={fileView(user.image._id)} alt="Profile" />
                          <div className="pt-2">
                            <label
                              for="uploadImage"
                  ...
                  >
                    <form onSubmit={handleChangePass}>
                      <div className="row mb-3">
                        <label
                          for="currentPassword"
                          className="col-md-6 col-lg-4 col-form-label"
                        >
                          Mot de passe actuel :
                        </label>
